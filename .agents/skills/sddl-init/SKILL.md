...
6. Skill installation
   ...
   **If copy:**
   ...
   - In each copied `.md` file, rewrite package-relative path prefixes to be project-relative:
     ...
     - `sdd-lite/orchestrator/` → `<package-root>/orchestrator/`
     - `sdd-lite/templates/artifacts/` → `<package-root>/templates/artifacts/`
     - `sdd-lite/templates/bootstrap/` → `<package-root>/templates/bootstrap/`
...
- wrapper blocks use contract version `0.4`, point to `sdd-lite/orchestrator/SDDL-RUNTIME.md`, contain worker-bypass controls, and name `execution_profile`
