...
When a gate needs the user, ask before writing the artifact. Use checkpoint type `missing_context` per `sdd-lite/skills/_shared/sddl-user-interaction-contract.md`. Asking shapes what goes into the artifact; it never replaces writing it.
...
Use `sdd-lite/templates/artifacts/proposal.md` as the baseline shape.
...
Before returning, apply the `phase_validation` checkpoint as defined in `sdd-lite/skills/_shared/sddl-user-interaction-contract.md`. It is conditional: skip it when the user already indicated advancement, and always present it when scope, risk, or interpretation is ambiguous, or when the artifact carries open questions or risks above medium severity.
...
Return the common result structure from `sdd-lite/skills/_shared/sddl-flow-contract.md`.
