...
Goal: the 5 rules from PRD §4.5 automatically verified.
...
Goal: nothing enters main without passing quality + guards.
...
🔴 required · Depends on: E0.F1.H1
Goal: the central contract that decouples all spike development.
...
Goal: develop and test the entire MVP without real engines.
...
🔴 required · Depends on: —
Goal: resolve the 4 points from PRD §6.2 for Claude Code.
...
Goal: feed contract tests with real outputs, not invented ones.
...
Goal: data for the context spike (PRD §6.3) without blocking anything.
...
🔴 required · Depends on: E0.F1.H1
Goal: implementation of the `GitPort` port for repo operations.
...
Goal: the central operations of the review flow.
...
🔴 required · Depends on: E0.F1.H1
Goal: all configuration validated and typed from disk.
...
Goal: `registerRepo` use case — the product's entry door.
...
Goal: `listRepos` and `listBranches` use cases (with prior fetch).
...
Goal: complete registration lifecycle management.
...
Goal: isolation guaranteed per review.
...
🔴 required · Depends on: E2.F1.H2
Goal: diff ready for the prompt with the PRD's warning policy.
...
Goal: the registry of review types available to the core.
...
Goal: same input → same prompt, auditable in history.
...
Goal: leave the door open for autonomous mode without implementing it (PRD §6.3).
...
🔴 required · Depends on: E3.F1.H1
Goal: the product's main harness.
...
Goal: review focused on vulnerabilities and sensitive data handling.
...
🔴 required · Depends on: E3.F2.H1
Goal: lightweight review for fast feedback.
...
Goal: leverage existing conventions of the reviewed repo.
...
Goal: the product's central flow, complete against FakeEngine.
...
🔴 required · Depends on: E0.F2.H1
Goal: the reliable output contract (success criterion ≥90% parsed).
...
Goal: first real engine behind the port.
...
Goal: second engine — validates the interface is genuine.
...
Goal: engine switch from PRD §3.1-D.
...
🔴 required · Depends on: E0.F1.H1
Goal: safe execution of declared processes.
...
Goal: repo scripts feed the review context (PRD §3.1-E).
...
Goal: self-contained run (requirement for the future daemon).
...
Goal: `listRuns` and `getRun` use cases.
...
Goal: cost visibility if the engine exposes it.
...
Goal: scriptable surface of the product.
...
Goal: use case 6 from the PRD (scripting) and seed of gate mode.
...
Goal: the main interactive flow (PRD §3.1-G).
...
Goal: readable result on completion + file path.
...
Goal: dig into findings with the agent in context (PRD §3.1-G ⚪).
...
Goal: safety net for the entire flow.
...
Goal: validate success criteria with own real PRs.
...
🔴 required · Depends on: E6.F1.H1
Goal: anyone installs and configures without this context conversation.
...
🔴 required · Depends on: —
Goal: close open decision 6 from the PRD before publishing.
