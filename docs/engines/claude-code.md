...
- **Input path chosen: stdin.** No practical size limit and no shell-escaping hazards with
  arbitrary diff content. Alternative (prompt as argv) works identically but hits the OS
  `execve` limit: on macOS (`ARG_MAX` = 1 MiB) exec fails at **~1024 KiB** of argv (shell
  exit 127, "argument list too long"); 1020 KiB still passes. Argv is viable for small
  prompts but stdin is strictly safer.
