...
- **A provenance note appended to `fixtures/README.md`.** That file currently opens by framing the entire `fixtures/` tree as "Real captured outputs from the E1 engine spikes", with no qualification. Adding a hand-written `fixtures/synthetic/` subdirectory makes that framing inaccurate, so the README gains a short section distinguishing the real captured corpus (12 files, E1.F1.H3, the `>=90%` denominator) from the synthetic parser fixtures (3 files, hand-written for this story, explicitly NOT part of that denominator). Doc-only; required so the change does not leave a stale claim behind.
...
4. **Empty/absent-input tolerance.** The parser never throws. Empty string, non-string/undefined-ish input coerced to a string, and unstructured non-JSON garbage text (e.g. a raw log dump) all resolve to `null` cleanly. Two distinct sub-cases, which the corpus keeps separate and a test-writer must not conflate:
   - *Genuinely empty reconstructed text*: `claude-code/timeout-sigterm.json` (no `.result` field at all), `opencode/context-overflow.ndjson` (error events only, no `text` events), `opencode/timeout-sigterm-partial.ndjson` (truncated mid-stream).
   - *Non-empty real content with no marker*: `claude-code/no-verdict.json` (full prose review), **`opencode/no-verdict.ndjson`** (two `text` events, ~449 chars of real prose — **corrected at spec-stage validation**; proposal.md's original inventory row wrongly described this as having no `text` events, and that row has been corrected in place), `claude-code/auth-error.json` / `claude-code/context-overflow.json` (short error strings), `opencode/unknown-model-stdout.txt` (raw non-JSON log dump). These prove the parser returns `null` on real text rather than false-positiving — a different property from empty-input tolerance, and the more valuable of the two.
...
1. **`fixtures/synthetic/decoy-then-genuine.txt`** — proves the tail-window provenance rule resolves to the genuine, tail-positioned verdict and ignores an earlier decoy of the same marker shape.
   - Structure: line 1 is a standalone decoy line reading exactly `VERDICT: comment` (marker-shaped, but a value distinct from the genuine one), followed by filler prose (any content not matching the marker regex — e.g. a repeated placeholder sentence discussing unrelated implementation detail), followed by a final line reading exactly `VERDICT: approve`.
   - **Sizing constraint — BOTH bounds are mandatory and neither implies the other.** The filler section must be **at least 55 lines AND at least 2200 characters** (i.e. filler lines averaging ~40+ characters). Rationale, and the trap this closes: rule 1 takes the *larger* of the two windows. Satisfying only the line count with short filler (say 55 one-word lines totalling under 2000 characters) makes the whole file shorter than 2000 characters, so the last-2000-characters window degenerates to **the entire file**, wins the union, pulls the decoy back into the scanned window, trips the contradiction rule, and the fixture resolves to `null` — **failing AC-3 instead of passing it**. The two bounds together guarantee line 1 falls outside both windows; the genuine final line falls inside both by construction.
   - Build check (run it, do not assume): after writing the file, confirm `wc -l` > 56 and `wc -c` > 2250, and confirm the decoy line's character offset from EOF exceeds 2000.
...
| AC-12 | No change to `runReview`'s executable code, the `deps.parseVerdict ?? extractBuiltInVerdict` line, or the `Verdict`/`VerdictParser` type contracts — only doc-comment prose changes in the four named files | `git diff` on `run-review.ts`, `index.ts` and `verdict.ts` shows comment-only hunks (all three) | must |
...
| AC-15 | No scope leak: the diff touches only `src/core/run/builtin-verdict-extraction.ts`, doc-comments in `run-review.ts`/`index.ts`/`verdict.ts`, `fixtures/synthetic/**`, the provenance note in `fixtures/README.md`, and `src/core/run/__test__/**` | `git diff --stat` | must |
