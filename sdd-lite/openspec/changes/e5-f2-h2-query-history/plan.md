...
| ST-5 | **Fix stage** (review-ledger.md R3-001, R4-001, both confirmed CRITICAL, `cp-review-gate`/`dec-005` authorized): (1) R3-001 — `get()`'s `"missing"` branch now also checks `exists(finalDir)` before falling back to the staging-sibling check, so a final directory that exists but lost its `metadata.json` correctly throws `RunCorruptedError` (matching `list()`'s classification of the identical state) instead of `RunNotFoundError`; (2) R4-001 — `list()`'s per-entry `readMetadata` call now catches a raw (non-ENOENT) fs failure and classifies that single entry as `corrupt` rather than rethrowing out of the loop and aborting the whole `list()` call, resolving the AC-7/AC-14 tension in favor of AC-7's "none affecting the others" per the review's explicit finding. New regression tests for both, added to the existing planted-state suite | ST-4 | `src/adapters/driven/storage/run-store-fs.ts` (edit), `src/adapters/driven/storage/__test__/run-store-fs.test.ts` (edit) | `npm run check` + `npm test` green; new tests: (a) `get()` on a final dir with no `metadata.json` and no `.partial-` sibling rejects with `RunCorruptedError`, not `RunNotFoundError` (R3-001 regression); (b) `list()` with one entry whose metadata read raises a mocked non-ENOENT error still returns every other entry correctly and classifies that one entry `corrupt` rather than rejecting the whole call (R4-001 regression). Scoped re-review against the frozen `review-ledger.md` verifies both fix deltas resolve their findings and touch nothing else | yes | yes | pending |
