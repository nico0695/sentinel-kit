...
`runTuiFlow` awaited `offerFullView` unguarded at both call sites, so a throw reached `createTui`'s catch-all and returned **1** for a run that had completed and been persisted — contradicting the module's own Property 5 and AC-10. The ledger recorded `R4-001` as `info` because R3 had refuted it for the shipped prompter seam (`@clack/core`'s promise takes no reject parameter, so `confirm` cannot reject). **R3 was right and the finding still stood**: the print loop inside `offerFullView` calls `io.stdout`, which is `process.stdout.write` in the real container and throws on EPIPE. That path needs no double swap at all — `sentinel | head`, a closed pager, a terminal that goes away mid-print.
